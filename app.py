...
# Built exactly same model while training, so we can load the weights without issues
...
def preprocess(q):
    ...
    contractions = {
        ...
        "it's": "it is", "let's": "let us", "she'd": "she would",
...
# exactly same padding/truncation logic as during training, so we can ensure consistent input lengths
...
def predict(question1, question2, model, vocab, device,
    ...
    q1_tokens = word_tokenize(preprocess(question1))
    q2_tokens = word_tokenize(preprocess(question2))

    q1_ids = pad_or_truncate([vocab.get(t, 0) for t in q1_tokens], max_len_q1)
    q2_ids = pad_or_truncate([vocab.get(t, 0) for t in q2_tokens], max_len_q2)
